...
namespace GestionGUIHelper.Helpers
{
    public static class MensajePorPantalla
    {
        ...
        {
            return MessageBox.Show(string.Format("El valor ingresado en el campo {0} debe ser numérico", controlName));
        }

        public static DialogResult MensajeValidacionString(string controlName, int minSize, int maxSize)
        {
            return MessageBox.Show(string.Format("El valor ingresado en el campo {0} debe tener entre {1} y {2} caracteres", controlName, minSize, maxSize));
        }

        public static DialogResult MensajeValidacionDateTime(string controlName, DateTime until)
        {
            return MessageBox.Show(string.Format("El valor ingresado en el campo {0} debe ser menor a la fecha {1}", controlName, FechaHelper.Format(until)));
        }
        ...
        public static DialogResult MensajeValidacionDateTime(string controlName, DateTime from, DateTime until)
        {
            ...
            return MessageBox.Show(string.Format("El valor ingresado en el campo {0} debe ser mayor a {1} y menor a {2}", controlName, sFrom, sUntil ));
        }
    }
}
